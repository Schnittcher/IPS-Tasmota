...
class TasmotaService extends IPSModule
{
    ...
    protected function traverseArray($array, $GesamtArray)
    {
        foreach ($array as $key=> $value) {
            if (is_array($value)) {
                ...
                if ($ParentKey == 'RC522') {
                    $variablenID = $this->RegisterVariableString('Tasmota' . $ParentKey . '' . $key, $ParentKey . '' . $key, '');
                    SetValue($this->GetIDForIdent('Tasmota' . $ParentKey . '_' . $key), $value);
                }
            }
        }
    }
    ...
    protected function getInfo2Variables($myBuffer)
    {
        if (is_object($myBuffer)) {
            ...
            SetValue($this->GetIDForIdent('Tasmota_IPAddress'), $myBuffer->IPAddress);
        }
    }
}