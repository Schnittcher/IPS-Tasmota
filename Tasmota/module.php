                    ...
                    if (fnmatch('*ShutterLock*', $Buffer->Payload)) {
                        ...
                        for ($i = 1; $i <= 5; $i++) {
                            if (property_exists($Payload, 'ShutterLock' . $i)) {
                                $this->RegisterVariableBoolean('Tasmota_ShutterLock' . $i, 'Tasmota_ShutterLock ' . $i, '~Lock', 0);
                    ...
                    for ($i = 0; $i <= 5; $i++) {
                        if (property_exists($Payload, 'Shutter' . $i)) {
                            if (property_exists($Payload->{'Shutter' . $i}, 'Position')) {
                                ...
                                $this->SetValue('Tasmota_Shutter' . $i . '_Position', $Payload->{'Shutter' . $i}->{'Position'});
                            }
                            if (property_exists($Payload->{'Shutter' . $i}, 'Direction')) {
                                ...
                                $this->SetValue('Tasmota_Shutter' . $i . '_Direction', $Payload->{'Shutter' . $i}->{'Direction'});
                            }
                            if (property_exists($Payload->{'Shutter' . $i}, 'Target')) {
                                $this->RegisterVariableInteger('Tasmota_Shutter' . $i . '_Target', 'Shutter' . $i . '_Target', '', 0);
                                $this->SetValue('Tasmota_Shutter' . $i . '_Target', $Payload->{'Shutter' . $i}->{'Target'});
                            }
                        }
                    }
                    return;
